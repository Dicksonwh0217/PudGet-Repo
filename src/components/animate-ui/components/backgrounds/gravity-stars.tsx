...
export function GravityStarsBackground({
  ...
  const updateStars = React.useCallback(() => {
    ...
    for (let i = 0; i < starsRef.current.length; i++) {
      ...
      if (dist < mouseInfluence && dist > 0) {
        ...
        if (glowAnimation === "instant") p.glowMultiplier = targetGlow
        else if (glowAnimation === "ease") p.glowMultiplier = currentGlow + (targetGlow - currentGlow) * 0.15
        else {
        ...
        if (glowAnimation === "instant") p.glowMultiplier = targetGlow
        else if (glowAnimation === "ease") p.glowMultiplier = Math.max(1, currentGlow + (targetGlow - currentGlow) * 0.08)
        else {
  ...
  }, [animate, initStars, canvasSize.width, canvasSize.height])
